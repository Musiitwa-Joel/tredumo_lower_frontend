...
import { GET_SCHEMES } from "../../../graphql/queries";
...
import { DELETE_SCHEME } from "../../../graphql/mutations";
...
const rows = [
  ...
  },
  {
    id: "Mature Age Entry",
    last_modified_by: "Joel Musiitwa",
    last_modified_date: "09/15/2021",
    created_by: "Akampa Darlington",
    creation_date: "2022-09-03T16:20:00",
  },
  {
    id: "Postgraduate",
    last_modified_by: "Akampa Darlington",
    last_modified_date: "08/20/2021",
    created_by: "Lubega Tasha",
    creation_date: "2022-08-18T12:10:00",
  },
  {
    id: "Certificates",
    last_modified_by: "Lubega Tasha",
    last_modified_date: "07/05/2021",
    created_by: "Joel Musiitwa",
    creation_date: "2022-07-05T09:55:00",
  },
];
...
function SchemesTable() {
  ...
  if (data) {
    dispatch(setSchemes(data.schemes));
  }
  ...
  const _schemes = useSelector(selectSchemes);
  const deleteDialogOpen = useSelector(selectDeleteSchemeDialogOpen);
  const selectedScheme = useSelector(selectSchemeToDelete);

  const handleClose = () => {
    dispatch(setDeleteSchemeDialogOpen(false));
  };
  ...
  const handleDelete = async () => {
    ...
    dispatch(
      showMessage({
        message: res.data.deleteScheme.message,
        variant: "info",
      })
    );
    // setOpen(false);
  ...
  const visibleRows = React.useMemo(
    () => stableSort(_schemes, getComparator(order, orderBy)),
    [_schemes]
  );
                        ...
                        <Tooltip title="Delete">
                          <Delete
                            onClick={() => {
                              ...
                              dispatch(setSchemeToDelete(row));
                              dispatch(setDeleteSchemeDialogOpen(true));
                            }}
                            style={{
                              color: "red",
                              fontSize: 18,
