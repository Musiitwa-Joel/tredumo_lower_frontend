...
function ClassesDataTable() {
  ...
  const [orderBy, setOrderBy] = React.useState("class_abbreviation");
  ...
  // On mount, if no classes in Redux, load dummy data
  useEffect(() => {
    if (!classes || classes.length === 0) {
      dispatch(updateClasses(rows));
    }
  }, [dispatch, classes]);
  ...
  // Ensure classes is an array before sorting
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - totalClasses) : 0;
              ...
              <TableBody>
                {visibleRows.map((row, index) => (
                  ...
                  >
                    ...
                    >
                      {Array.isArray(row.class_sections)
                        ? row.class_sections.join(", ")
                        : ""}
                    </TableCell>
                    <TableCell
                      align="right"
                    ...
                    >
                      <Tooltip title="Edit">
                        <Edit
                          onClick={() => {
                            dispatch(updateSelectedClass(row));
                          }}
                          style={{
                            color: "blue",
                            fontSize: 18,
