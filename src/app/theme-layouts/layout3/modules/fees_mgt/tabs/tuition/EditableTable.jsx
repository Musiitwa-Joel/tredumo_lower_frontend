...
import { DELETE_TUITION_FESS, SAVE_TUITION_FEE } from "../../gql/mutations";
...
const EditableTable = () => {
  ...
  const loadingTuitionFees = useSelector(selectLoadingTuitionFees);
  const tuitionFeesItems = useSelector(selectTuitionFeesItems);
  const selectedTuitionFeeItem = useSelector(selectSelectedTuitionFeeItem);
  ...
  ] = useMutation(SAVE_TUITION_FEE, {
    refetchQueries: ["tuitionFees"],
  });

  const [
    deleteTuitionFee,
    { error: deleteErr, loading: deletingTuitionFee, data: deleteRes },
  ] = useMutation(DELETE_TUITION_FESS, {
    refetchQueries: ["tuitionFees"],
  });
  ...
  const edit = (record) => {
    // console.log("record", record);
    dispatch(setSelectedTuitionFeeItem(record));
    form.setFieldsValue({
  ...
  const cancel = (record) => {
    ...
    if (record.frequency_code == "") {
      ...
      const updatedItems = tuitionFeesItems.slice(0, -1);
      dispatch(setTuitionFeesItems(updatedItems));
    }
  ...
  const save = async (key) => {
    try {
      ...
      const payload = {
        ...
        schoolId: selectedTreeItem?.details.school_id,
        courseId: selectedTreeItem?.details.course_id,
        studyYr: `${selectedTreeItem?.details.study_yr}`,
        ...
        saveTuitionFeeId: selectedTuitionFeeItem
          ? selectedTuitionFeeItem.id
          : null,
      };
      // const newData = [...data];
      ...
      const res = await saveTuitionFee({
        variables: payload,
      });

      dispatch(
        showMessage({
          message: res.data.saveTuitionFee.message,
          variant: "success",
        })
      );
  ...
  const handleAddItem = async () => {
    ...
    dispatch(setSelectedTuitionFeeItem(null));
    // Increment count based on the length of tuitionFeesItems
    setCount((prevCount) => prevCount + tuitionFeesItems.length + 1);
    ...
    const updatedItems = [...tuitionFeesItems, _d];
  ...
  const confirm = async (e, record) => {
    ...
    const payload = {
      tuitionFeeId: record.id,
    };

    const res = await deleteTuitionFee({
      variables: payload,
    });

    dispatch(
      showMessage({
        message: res.data.deleteTuitionFee.message,
        variant: "success",
      })
    );
  };
