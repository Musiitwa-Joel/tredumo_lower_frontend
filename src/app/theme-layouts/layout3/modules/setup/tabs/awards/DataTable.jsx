...
import { GET_AWARDS } from "../../gql/queries";
...
function DataTable() {
  ...
  const awards = useSelector((state) => state.setUp.awards);

  if (error) {
    dispatch(
      showMessage({
        message: "Failed to load awards " + error.message,
        variant: "error",
      })
    );
  }
  ...
  useEffect(() => {
    if (data) {
      ...
      dispatch(updateAwards(data.awards));
    }
  }, [data]);

  const visibleRows = React.useMemo(
    () =>
      stableSort(awards, getComparator(order, orderBy)).slice(
        page * rowsPerPage,
        page * rowsPerPage + rowsPerPage
      ),
    [order, orderBy, page, rowsPerPage, awards]
  );
                        ...
                        <Tooltip title="Edit">
                          <Edit
                            onClick={() => {
                              ...
                              dispatch(updateSelectedAward(row));
                            }}
                            style={{
                              color: "blue",
                              fontSize: 18,
