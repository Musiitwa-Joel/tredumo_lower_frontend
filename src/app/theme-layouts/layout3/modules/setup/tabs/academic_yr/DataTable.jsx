...
import { GET_ACC_YRS } from "../../gql/queries";
...
function DataTable() {
  ...
  const acc_yrs = useSelector((state) => state.setUp.acc_yrs);

  if (error) {
    dispatch(
      showMessage({
        message: "Failed to load acc yrs " + error.message,
        variant: "error",
      })
    );
  }

  // console.log("acc yrs", acc_yrs);
  ...
  const visibleRows = React.useMemo(
    () =>
      stableSort(acc_yrs, getComparator(order, orderBy)).slice(
        page * rowsPerPage,
        page * rowsPerPage + rowsPerPage
      ),
    [order, orderBy, page, rowsPerPage, acc_yrs]
  );
                        ...
                        <Tooltip title="Edit">
                          <Edit
                            onClick={() => {
                              ...
                              dispatch(updateSelectedAccYr(row));
                            }}
                            style={{
                              color: "blue",
                              fontSize: 18,
