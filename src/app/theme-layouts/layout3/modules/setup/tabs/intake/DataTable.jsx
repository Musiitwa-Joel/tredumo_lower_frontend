...
import { GET_INTAKES } from "../../gql/queries";
...
function DataTable() {
  ...
  const intakes = useSelector((state) => state.setUp.intakes);

  if (error) {
    dispatch(
      showMessage({
        message: "Failed to load intakes " + error.message,
        variant: "error",
      })
    );
  }
  ...
  useEffect(() => {
    if (data) {
      ...
      dispatch(updateIntakes(data.intakes));
    }
  }, [data]);

  const visibleRows = React.useMemo(
    () =>
      stableSort(intakes, getComparator(order, orderBy)).slice(
        page * rowsPerPage,
        page * rowsPerPage + rowsPerPage
      ),
    [order, orderBy, page, rowsPerPage, intakes]
  );
                        ...
                        <Tooltip title="Edit">
                          <Edit
                            onClick={() => {
                              ...
                              dispatch(updateSelectedIntake(row));
                            }}
                            style={{
                              color: "blue",
                              fontSize: 18,
