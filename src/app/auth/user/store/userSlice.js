...
export const { userSignOut, updateUser, setUserShortcuts, setUserSettings } =
  userSlice.actions;
export const selectUser = (state) => state?._user;
export const selectUserId = (state) => state?._user?.uid;
export const selectUserRole = (state) => state?._user?.role;
export const selectIsUserGuest = (state) => {
  const userRole = state?.user?.role;
  return !userRole || userRole?.length === 0;
};
export const selectUserShortcuts = (state) => state._user?.data?.shortcuts;
export const selectUserSettings = (state) => state._user?.data?.settings;
